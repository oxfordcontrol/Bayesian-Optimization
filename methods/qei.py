...
class QEI(BO):
    ...
    def acquisition_tf(self, X):
        ...
        if self.kern.ARD:
            ...
            cov_param = np.repeat(self.kern.lengthscales.value, X.shape[1])
        ...
        if self.options['mean_function'] is None:
            r_model = robjects.r['create_model'](
            ...
            assert A.size == 1
            r_model = robjects.r['create_model_1d_quadratic_mean'](
                self.X.value, self.Y.value,
                cov_type, cov_param, cov_var, var,
                A[:]
            )

        qEI = robjects.r['qEI_caller'](X, r_model)
        grad = robjects.r['qEI_grad_caller'](X, r_model)
